...
def _fetch_checklist_items_for_term(conn, term: str, year: int):
    """
    Returns list of dicts:
      {id, area, area_code, section, label, competence, sort_order}
    ...
    cur.execute("""
      ...
      JOIN robotics_checklist_item_terms t ON t.item_id = i.id
      WHERE i.is_active=1
        AND t.is_active=1
        AND t.term=%s
        AND (t.year IS NULL OR t.year=%s)
...
def competency_checklist(student_id):
    ...
    try:
        ...
        # Use SAME conn (no reconnect / no overwrite)
        items = _fetch_checklist_items_for_term(conn, term, year)  # list of dicts

        # saved ticks/remarks for this term/year (reuse SAME conn if your helper supports it)
        # If your helper doesn't accept conn, it's still okay, but better to pass it in.
        try:
        ...
        if meta.get("next_term_end"):
            meta["next_term_end"] = meta["next_term_end"].strftime("%Y-%m-%d")  # <-- fixed typo

        # NOTE: prev_id / next_id must be defined somewhere; if not, remove them or set None
        prev_id = None
        next_id = None
...
@app.route("/admin/checklist_items", methods=["GET", "POST"])
@require_role("admin", "headteacher")
...
def character_form():
    ...
    # ✅ IMPORTANT: fetch student BEFORE using student.class_name anywhere
    student = _fetch_student_basic(student_id)
    if not student:
        flash("Learner not found.", "danger")
        return redirect(url_for("students"))

    # ✅ Term-based items (and optionally class-based)
    items = _fetch_character_items(term, student.get("class_name"))
    if not items:
        flash("No character assessment items defined for this term/class.", "warning")
    ...
    if meta.get("next_term_begin") and hasattr(meta["next_term_begin"], "strftime"):
        meta["next_term_begin"] = meta["next_term_begin"].strftime("%Y-%m-%d")
    if meta.get("next_term_end") and hasattr(meta["next_term_end"], "strftime"):
        meta["next_term_end"] = meta["next_term_end"].strftime("%Y-%m-%d")

    conn = get_db_connection()

    # 1) auto-fill SPECIAL COMMUNICATION from overrides
    try:
        overrides = fetch_overall_overrides(conn, student_id, term, year)
    except Exception:
        overrides = {}

    if overrides and not (meta.get("special_communication") or "").strip():
        meta["special_communication"] = (
            overrides.get("special_communication")
            or meta.get("special_communication")
            or ""
        )

    # 2) auto-fill NEXT TERM DATES from helper
    try:
